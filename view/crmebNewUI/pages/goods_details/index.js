...
Page({
  ...
  */
  goCat:function(isPay){
    ...
    if (this.data.attrValue){
      this.setData({ 'attribute.cartAttr': false }) 
    }else{
      if (this.data.isOpen) 
  ...
  */
  downloadFilePromotionCode: function (successFn) {
    ...
    app.baseGet(app.U({ c: 'store_api', a:'product_promotion_code',q:{id:this.data.id}}),function(res){
      wx.downloadFile({url: that.setDomain(res.msg),success: function (res) { 
  ...
  */
  goPoster:function(){
    ...
    const arr2 = [that.data.posterbackgd, that.data.storeImage, that.data.PromotionCode];
    if (arr2[2] == ''){
      //海报二维码不存在则从新下载
